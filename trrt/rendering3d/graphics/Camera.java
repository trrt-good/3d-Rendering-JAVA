...
public class Camera
{
    ...
    {
        private int maxDistance = (int)farClipDistance; //maximum distance the camera can be from the object
        ...
        public OrbitCamController(GameObject focusObjectIn, double sensitivityIn, double scrollSense)
        {
        ...
        {
            distance = Math.max(minDistance, Math.min(distance + e.getWheelRotation()*scrollSensitivity, maxDistance));
            difference = Vector3.multiply(directionUnit, distance);
            updatePosition();
        }
    ...
    public void setOrbitControls(JPanel panel, GameObject focusObject, double sensitivity, double scrollSens)
    {
        orbitController = new OrbitCamController(focusObject, sensitivity, scrollSens);
